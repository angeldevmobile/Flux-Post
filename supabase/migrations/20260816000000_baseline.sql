...
--
-- El esquema tal y como ya estaba desplegado antes de que existieran las
-- migraciones versionadas. Es la union literal de los antiguos
-- `supabase/schema.sql` y `supabase/ai_usage.sql`, sin cambios de contenido:
-- este fichero no introduce nada nuevo, solo fija el punto de partida.
--
-- Es idempotente de principio a fin (`if not exists`, `or replace`,
-- `drop policy if exists`), asi que aplicarlo sobre el proyecto de produccion
-- que ya lo tiene todo no hace nada. No hace falta `migration repair`.
-- ═══════════════════════════════════════════════════════════════
